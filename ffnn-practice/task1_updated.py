            ...
            if count > 0:
                ...
                if len(line) > 10:
                    ...
                    if tokens[3] == 'Good':
                        ...
                        cls = 2
                    data.append([x1, x2, x3, cls])
            count += 1
    return data

def load_data(file_path):
    data = read_insurability(file_path)
    X = [d[:-1] for d in data]
    y = [d[-1] for d in data]
...
# Load the dataset
train_X, train_y = load_data('three_train.csv')
valid_X, valid_y = load_data('three_valid.csv')
test_X, test_y = load_data('three_test.csv')
# Create data loaders
batch_size = 2
