...
def read_mnist(file_name):
    ...
    with open(file_name, 'rt') as f:
        for line in f:
            ...
            tokens = line.split(',')
            label = tokens[0]
            attribs = [int(token) for token in tokens[1:785]]
            data_set.append([label] + attribs)
    return data_set

def load_mnist_data(file_path, transform=None):
    data = read_mnist(file_path)
    X = np.array([x[1:] for x in data], dtype=np.float32) / 255.0  # Normalizing the pixel values
    y = np.array([x[0] for x in data], dtype=np.int64)
    if transform:
        X = transform(X)
    return torch.tensor(X, dtype=torch.float32), torch.tensor(y, dtype=torch.long)
# Load the MNIST dataset
mnist_train_X, mnist_train_y = load_mnist_data('mnist_train.csv')
mnist_valid_X, mnist_valid_y = load_mnist_data('mnist_valid.csv')
mnist_test_X, mnist_test_y = load_mnist_data('mnist_test.csv')
batch_size = 2
...
weight_decay = 1e-5 
mnist_optimizer = optim.Adam(mnist_model.parameters(), lr=mnist_learning_rate, weight_decay=weight_decay)
...
# function to train the network and record losses and accuracies
